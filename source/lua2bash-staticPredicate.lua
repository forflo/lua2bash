...
function statcheck.statcheck.isStaticSet(ast)
    local varlist = ast[1]
    local explist = ast[2]
...
function statcheck.statcheck.isStaticFor(ast)
    if #ast == 5 then
...
function statcheck.statcheck.isStaticForIn(ast)
    local namelist = ast[1]
    local explist = ast[2]
    local block = ast[3]
    return statcheck.isStaticNamelist(namelist) and
...
function statcheck.statcheck.isStaticWhile(ast)
    return statcheck.isStaticExp(ast[1]) and statcheck.isStaticBlock(ast[2])
end

function statcheck.statcheck.isStaticIf(ast)
    local result = true
    local elseB = #ast % 2
...
function statcheck.statcheck.isStaticRepeat(ast)
    return statcheck.isStaticBlock(ast[1]) and statcheck.isStaticBlock(ast[2])
end

function statcheck.statcheck.isStaticDo(ast)
    local result = true
    imap(ast, function(node) result = result and statcheck.isStaticStmt(node) end)
    return result
end

function statcheck.statcheck.isStaticRet(ast)
    return util.ifold(
        util.imap(
            ast,
            function(exp)
...
function statcheck.statcheck.isStaticStm(ast)
...
function statcheck.statcheck.isStaticBlock(ast)
    local result = true
    imap(ast, function(node) result = result and statcheck.isStaticStmt(node) end)
    return result
end

function statcheck.statcheck.isStaticExp(ast)
    if ast.tag == "Op" then
